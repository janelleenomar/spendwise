...
class _HomeScreenState extends State<HomeScreen> {
  // null means show ALL categories; a specific value filters the list
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // The ValueListenableBuilder listens to the Hive box and rebuilds automatically
      body: ValueListenableBuilder<Box<Expense>>(
        valueListenable: ExpenseService.listenable,
        builder: (context, box, _) {
          // Recalculate the total every time the box changes
          final double total = box.values.fold(0.0, (s, e) => s + e.amount);
          
          // Get the expenses based on the selected filter
